  /// @notice LOTUSTEST code
  /// @notice Researched and modified by zzlnc4
  /// @notice Introduced to the dev-team on October 29, 2017
...
contract LOTUSx is StdToken
{
/// Fields:
    string public constant name = "LOTUSX";
    string public constant symbol = "LTX";
    uint public constant decimals = 4;

    // this includes DEVELOPERS_BONUS
    uint public constant TOTAL_SUPPLY = 85000000 * (1 ether / 1 wei);
    uint public constant DEVELOPERS_BONUS = devBonus * (1 ether / 1 wei);

    uint public constant PRESALE_PRICE = 3000;  // per 1 Ether
    uint public constant PRESALE_MAX_ETH = 1417;
    // 8.5 M LTK tokens sold during PRESALE PERIOD
    uint public constant PRESALE_TOKEN_SUPPLY_LIMIT = PRESALE_PRICE * PRESALE_MAX_ETH * (1 ether / 1 wei);

    uint public constant ICO_PRICE1 = 3000;     // per 1 Ether is 3000LTK
    uint public constant ICO_PRICE2 = 2000;     // per 1 Ether is 2000LTK
    uint public constant ICO_PRICE3 = 1000;     // per 1 Ether is 1000LTK

    // 85M- this includes presale tokens
    uint public constant TOTAL_SOLD_TOKEN_SUPPLY_LIMIT = 85000000* (1 ether / 1 wei);
...
/// Functions:
    ...
    function LOTUSx(address _tokenManager, address _escrow, address _teamTokenBonus)
    {
        ...
        assert(PRESALE_TOKEN_SUPPLY_LIMIT==8500000 * (1 ether / 1 wei));
        assert(TOTAL_SOLD_TOKEN_SUPPLY_LIMIT==25500000 * (1 ether / 1 wei));
    }
    ...
    function getPrice()constant returns(uint)
    {
        if(currentState==State.ICORunning){
             if(icoSoldTokens<(12750000 * (1 ether / 1 wei))){
                  return ICO_PRICE1;
             }
